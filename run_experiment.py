...
from rl.envs.wrappers2 import SymmetricEnv
...
def import_env(env_name_str):
    if env_name_str=='h1':
        from envs.h1.h1_env import Quadrotor_Env as Env 
    else:
    ...
    if args.mirror:
        try:
            ...
            env_fn = partial(SymmetricEnv, env_fn,
                             mirrored_obs=mirrored_obs,
                             mirrored_act=mirrored_act)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == 'train':
        ...
        parser.add_argument("--input-norm-steps", type=int, default=10000)
        parser.add_argument("--n-itr", type=int, default=4000, help="Number of iterations of the learning algorithm")
        ...
        parser.add_argument("--entropy-coeff", type=float, default=0.01, help="Coefficient for entropy regularization")
        ...
        parser.add_argument("--num-procs", type=int, default=8, help="Number of threads to train on")
        parser.add_argument("--max-grad-norm", type=float, default=0.1, help="Value to clip gradients at")
        parser.add_argument("--max-traj-len", type=int, default=400, help="Max episode horizon")
        parser.add_argument("--mirror", action="store_true", help="to use SymmetricEnv")
        parser.add_argument("--mirror-coeff", required=True, default=0.5, type=float, help="weight for mirror loss")

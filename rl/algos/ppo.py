...
class PPO:
    ...
    def update_actor_critic(self, obs_batch, action_batch, return_batch, advantage_batch, mask, mirror_observation, mirror_action):
        ...
        #entropy_penalty = -(pdf.base_dist.entropy() * mask).mean()
        
        # Mirror Symmetry Loss TODO
        deterministic_actions = self.policy(obs_batch)
        if mirror_observation and mirror_action:
            if self.recurrent:
    ...
    def train(self, env_fn, n_itr):
        ...
        self.actor_optimizer = optim.AdamW(self.policy.parameters(), lr=self.lr, eps=self.eps)
        self.critic_optimizer = optim.AdamW(self.critic.parameters(), lr=self.lr, eps=self.eps)

        train_start_time = time.time()

        # obs_mirr, act_mirr = None, None  # definite in h1_env.py
        ...
        for itr in range(n_itr):
            ...
            for epoch in range(self.epochs):
                ...
                for indices in sampler:
                    ...
                    if approx_kl_div > self.kl_cutoff:
                        print(f"Early stopping at update {epoch} due to KL divergence = {approx_kl_div:.4f}")
                        break
            ...
            if mean_kl > 1.5 * self.target_kl:
                self.actor_lr = max(self.actor_lr / 2, self.min_lr)
            elif mean_kl < 0.5 * self.target_kl:
                self.actor_lr = min(self.actor_lr * 2, self.max_lr)
        ...
        def plot_tensorboard_scalars(log_dir, scalar_list, save_dir="plots", title_prefix=""):
          """
          Read scalar series from TensorBoard logs and save each plot.
    
          :param log_dir: TensorBoard log directory as a string path.
          :param scalar_list: scalar names to plot, such as ["Loss/actor", "Train/mean_reward"].
          :param save_dir: output directory for generated plots.
          :param title_prefix: plot title and filename prefix, such as "PPO_".
          """
          log_dir = str(log_dir)
          ...
          for scalar in scalar_list:
              if scalar not in ea.Tags()["scalars"]:
                  print(f"[Warning] Scalar '{scalar}' not found in log.")
                  continue
              ...
              plt.figure()
              plt.plot(steps, values, label=scalar, color='blue')
              plt.xlabel("Step")
              plt.ylabel(scalar)
              plt.title(f"{title_prefix}{scalar}")
              plt.grid(True)
              plt.legend()

              file_name = scalar.replace("/", "_") + ".png"
              file_path = os.path.join(save_dir, title_prefix + file_name)
        ...
        plot_tensorboard_scalars(
           ...
           save_dir="plots/ppo_experiment10",
           title_prefix="PPO_"
         )

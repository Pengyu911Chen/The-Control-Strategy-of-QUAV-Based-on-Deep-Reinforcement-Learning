...
class TanhNormal:
    ...
    def log_prob(self, value):
        atanh_value = 0.5 * torch.log((1 + value + self.epsilon) / (1 - value + self.epsilon))
        log_prob = self.normal.log_prob(atanh_value)
        # log |dy/dx| = log(1 - tanh(x)^2) = log(1 - y^2)
        log_det_jacobian = 2 * (torch.log(torch.tensor(2.)) - value - F.softplus(-2 * value))
...
class Gaussian_FF_Actor(Actor):  # more consistent with other actor naming conventions
    def __init__(self, state_dim, action_dim, layers=(256, 256), nonlinearity=torch.nn.functional.relu,
                 init_std=0.1, learn_std=True, bounded=False, normc_init=True):
    ...
    #def make_tanh_normal(self,mu, std):
...
class Gaussian_LSTM_Actor(Actor):
    def __init__(self, state_dim, action_dim, layers=(128, 128), nonlinearity=F.tanh, normc_init=True,
                 init_std=0.1, learn_std=True):
    ...
    def _get_dist_params(self, state):
            ...
            for idx, layer in enumerate(self.actor_layers):
                ...
                for idx, layer in enumerate(self.actor_layers):
                    ...
                    if x.size(0) != h.size(0):
                       h = torch.zeros(x.size(0), h.size(1)).to(x.device)
                       c = torch.zeros(x.size(0), c.size(1)).to(x.device)
        ...
        mu = torch.tanh(self.network_out(x))
        sd = self.stds
        return mu, sd
